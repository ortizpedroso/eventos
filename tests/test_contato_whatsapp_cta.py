"""WhatsApp CTA em /contato — social_whatsapp_url ou contact_phone da plataforma."""
...
def test_contato_usa_cta_whatsapp_da_plataforma():
    ...
    assert "resolveWhatsappHref" in FOOTER
...
def test_navbar_admin_visivel_para_plataforma():
    assert "data-navbar-admin" in NAV
    assert "Administração" in NAV
    assert "isPlatformAdmin" in NAV
    assert "peekSessionCache()?.is_platform_admin" in NAV
