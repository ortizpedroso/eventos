...
if [[ -z "$DOMAIN" ]]; then
  ...
  echo "  - Transferências (saques): TRANSFER_CREATED, TRANSFER_PENDING, TRANSFER_IN_BANK_PROCESSING, TRANSFER_DONE, TRANSFER_FAILED, TRANSFER_CANCELLED"
  ...
  echo ""
  echo "Autorização de saques (BaaS, sem SMS): ./scripts/asaas-transfer-auth-setup.sh SEU_DOMINIO.com.br"
  exit 0
fi
